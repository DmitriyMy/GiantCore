...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CHAREDCOIN address (e.g. %1)").arg("GVXmy14r8hmuzt1d5ygnqzZfbuomg1XLmF"));
#endif
...
{
    // return if URI is not valid or is no CHAREDCOIN: URI
...
{
    // Convert charedcoin:// to charedcoin:
    //
    //    Cannot handle this later, because charedcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open charedcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CHAREDCOIN.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for CHAREDCOIN.lnk
...
{
    return GetAutostartDir() / "charedcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a charedcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CHAREDCOIN\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the charedcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add charedcoin app to startup item list
