...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CHAREDCOINMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CHAREDCOINMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CHAREDCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("charedcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("CHAREDCOINMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running CHAREDCOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadCHAREDCOINMiner() exception");
    } catch (...) {
        LogPrintf("ThreadCHAREDCOINMiner() exception");
    }

    LogPrintf("ThreadCHAREDCOINMiner exiting\n");
}
