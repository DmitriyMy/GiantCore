...
*Setup instructions for a gitian build of CHAREDCOIN using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CHAREDCOIN
...
- [Building ALQO](#building-charedcoin)
...
Clone the git repositories for charedcoin and gitian and then checkout the charedcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the charedcoin repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the charedcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/charedcoin/.git/
...
./bin/gbuild --commit charedcoin=${COMMIT} --url charedcoin=${URL} ../charedcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit charedcoin=${COMMIT} --url charedcoin=${URL} ../charedcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit charedcoin=${COMMIT} --url charedcoin=${URL} ../charedcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/charedcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/charedcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/charedcoin-build.assert
```
...
[charedcoin/gitian.sigs](https://github.com/charedcoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
